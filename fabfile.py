...
PROJECT_NAME = 'djangoprojectskeleton'
PROJECT_DATABASE_NAME = PROJECT_NAME.replace('-','')

# The name of the project on github
GIT_PROJECT_NAME = 'djangoprojectskeleton'
GIT_CLONE_PATH = 'juiceinc/%s.git' % GIT_PROJECT_NAME

DOMAIN = 'ec2-50-16-123-13.compute-1.amazonaws.com'
PRODUCTION_USERNAME = 'ubuntu'
PRODUCTION_HOST = DOMAIN # Change this to an IP if your DNS isn't resolving yet

ADMIN_EMAIL = 'chris@juiceanalytics.com'
env.hosts = [
    'ubuntu@ec2-50-16-123-13.compute-1.amazonaws.com'
]
env.user = 'ubuntu'
USE_CRONTAB = False
...
# github.com or git.assembla.com
GIT_CLONE_HOST = 'github.com'

GIT_CLONE_PSEUDOHOST = GIT_PROJECT_NAME # Used to specify site-specific behavior for SSH if multiple projects are hosted on e.g. github.com
PG_VERSION = (8, 4)
PYTHON_VERSION = (2, 7)
...
class Pip(object):
    ...
    def install_requirements():
        ...
        put('./setup/requirements/production.txt', REMOTE_FILENAME)
...
def install_common():
    ...
    #put('./setup/server/grub_preseed.cfg', 'grub_preseed.cfg')
    ...
    Apt.install('python-setuptools', 'python-pycurl', 'vim', 'screen', 'language-pack-en', 'git-core', 'python-jinja2',
...
def _key_name(public=True):
    return PROJECT_NAME + '_rsa' + ('.pub' if public else '')

def _key_destination(public=True):
    path = '.ssh/' + _key_name(public)
    return home_dir(path)

def install_keys():
    run('mkdir -p ~/.ssh')
    # Failing here? You need to generate a keypair and put it in the 'setup/server' directory.
    # The point is to use this as a GitHub (or other) 'deploy key'. Delete this stuff
    # if you don't want that.
    put('./setup/server/id_rsa', _key_destination(public=False))
    put('./setup/server/id_rsa.pub', _key_destination())
    run('chmod 600 %s' % _key_destination(public=False))

    # So we can git clone from git@github.com w/o manual confirmation:
    put('./setup/server/known_hosts', home_dir('.ssh/known_hosts'))
    ...
    for l in lines:
        assert "'" not in l
        run("echo '%s' >> %s/.ssh/config" % (l, home_dir()))
        run("echo '%s' >> %s/.ssh/config" % ('', home_dir()))
...
#    """
#    put('./setup/server/processor/compiler.jar', os.path.join(PROJECT_DIR, 'bin', 'compiler.jar'))
#    put('./setup/server/processor/processor', os.path.join(PROJECT_DIR, 'bin', 'processor'))
...
def install_smtp():
    ...
    put('./setup/server/postfix_preseed.cfg', 'postfix_preseed.cfg')
...
def configure_nginx():
    put('./setup/server/nginx/nginx.conf', '/etc/nginx/nginx.conf', use_sudo=True)
    upload_template( './setup/server/nginx/site', '/etc/nginx/sites-available/%s' % PROJECT_NAME,
...
def configure_django():
    upload_template('./setup/server/django/wsgi.py', os.path.join(PROJECT_DIR, 'wsgi.py'), use_jinja=True, context={
        'PROJECT_NAME': PROJECT_NAME,
        'PYTHON_VERSION_STR': "%d.%d" % PYTHON_VERSION,
    })

    upload_template('./setup/server/django/vhost', '/etc/apache2/sites-available/%s' % PROJECT_NAME, use_sudo=True, use_jinja=True, context={
        'DJANGO_PORT': DJANGO_PORT,
        'PROJECT_NAME': PROJECT_NAME,
    ...
    upload_template('./setup/server/django/ports.conf', '/etc/apache2/ports.conf', use_sudo=True, use_jinja=True, context={
        'DJANGO_PORT': DJANGO_PORT,
    })
    upload_template('./setup/server/django/stagesettings.py', os.path.join(PROJECT_DIR, 'stagesettings.py'), use_sudo=True,
        use_jinja=True, context={
        'database_host': '127.0.0.1', # Change this on switch to a multi-server setup
    })
...
def configure_database():
    ...
    for filename in ['environment', 'pg_ctl.conf', 'pg_hba.conf', 'pg_ident.conf', 'postgresql.conf', 'start.conf']:
        local_file = os.path.join('./setup/server/database', filename)
        remote_file = os.path.join(config_dir, filename)
        upload_template( local_file, remote_file, use_sudo=True, use_jinja=True, context={
            'PROJECT_NAME': PROJECT_DATABASE_NAME,
            'PG_VERSION_STRING': "%d.%d" % PG_VERSION,
        })
        sudo('chown %s:%s %s' % ('postgres', 'postgres', remote_file))
    run_with_safe_error("createdb %s" % PROJECT_DATABASE_NAME, 'some dumb error', use_sudo=True, user='postgres')
    run_with_safe_error("""psql -c "create user %s with createdb encrypted password '%s'" """ % (PROJECT_DATABASE_NAME, DB_PASS), "some dumb error", use_sudo=True, user='postgres')
    sudo("""psql -c "grant all privileges on database %s to %s" """ % (PROJECT_DATABASE_NAME, PROJECT_DATABASE_NAME), user='postgres')
...
#        run('ssh-add ~/.ssh/%s' % _key_name(public=False))
    ...
    @staticmethod
    def get_git_repo():
        print "getting get repo"
        print GIT_CLONE_USERNAME + '@' + GIT_CLONE_PSEUDOHOST + ':' + GIT_CLONE_PATH
        return GIT_CLONE_USERNAME + '@' + GIT_CLONE_PSEUDOHOST + ':' + GIT_CLONE_PATH

    @staticmethod
    def upload_new_release():
        Deploy.setup_ssh_keys()
    ...
    def prep_release(name):
        ...
        logs_dir = os.path.join(release_dir, PROJECT_NAME, 'logs')
        ...
        with cd(django_dir):
            ...
            run(with_ve + 'python manage.py syncdb --noinput --settings=settings.production')
            run(with_ve + 'python manage.py migrate --noinput --settings=settings.production')
            run(with_ve + 'python manage.py loaddata initial_data --settings=settings.production')
            run(with_ve + 'python manage.py collectstatic --noinput --settings=settings.production')
